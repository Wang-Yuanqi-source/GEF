...
class GNNDataset_criticalpath(InMemoryDataset): 
    ...
    def raw_file_names(self):
        return os.listdir('/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/diffeq1/') 

    @property
    def processed_file_names(self):
        if self.type == 'wo_inter':
            return ['diffeq1_wo_inter.pt']
        elif self.type == 'rrg':
            return ['diffeq1_rrg.pt']
        elif self.type == 'clb':
            return ['diffeq1_clb.pt']
        else:
            return ['diffeq1.pt']
    ...
    def process(self):
        report_df = pd.read_csv('/home/wllpro/llwang/yfdai/HRAE_paper/raw_dataset_results/diffeq1.csv')
        data_list = []

        for json_file in tqdm(self.raw_file_names, desc='Processing JSON Files'):
            if json_file.endswith('.json'):
                json_file_path = f'/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/diffeq1/{json_file}'
