...
class GNNDataset_criticalpath(InMemoryDataset): 
    ...
    def raw_file_names(self):
        return os.listdir('/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/raygentop/') 

    @property
    def processed_file_names(self):
        if self.type == 'wo_inter':
            return ['raygentop_wo_inter.pt']
        elif self.type == 'rrg':
            return ['raygentop_rrg.pt']
        elif self.type == 'clb':
            return ['raygentop_clb.pt']
        else:
            return ['raygentop.pt']
    ...
    def process(self):
        report_df = pd.read_csv('/home/wllpro/llwang/yfdai/HRAE_paper/raw_dataset_results/raygentop.csv')
        data_list = []

        for json_file in tqdm(self.raw_file_names, desc='Processing JSON Files'):
            if json_file.endswith('.json'):
                json_file_path = f'/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/raygentop/{json_file}'
