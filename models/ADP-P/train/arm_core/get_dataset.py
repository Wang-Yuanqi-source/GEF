...
class GNNDataset_criticalpath(InMemoryDataset): 
    ...
    def raw_file_names(self):
        return os.listdir('/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/arm_core/')  

    @property
    def processed_file_names(self):
        if self.type == 'wo_inter':
            return ['arm_core_wo_inter.pt']
        elif self.type == 'rrg':
            return ['arm_core_rrg.pt']
        elif self.type == 'clb':
            return ['arm_core_clb.pt']
        else:
            return ['arm_core.pt']
    ...
    def process(self):
        report_df = pd.read_csv('/home/wllpro/llwang/yfdai/HRAE_paper/raw_dataset_results/arm_core.csv')
        data_list = []

        for json_file in tqdm(self.raw_file_names, desc='Processing JSON Files'):
            if json_file.endswith('.json'):
                json_file_path = f'/home/wllpro/llwang/yfdai/HRAE_paper/raw_crit_paths/arm_core/{json_file}'
                ...
                if f'{arch_name}.xml' in os.listdir('/home/wllpro/llwang/yfdai/HRAE_paper/raw_dataset_archs/'):
                    ...
                    if (not label_row.empty and not pd.isna(label_row['Critical Path'].iloc[0])):
                        # get average_improvement as a label
                        ...
                        data = process_xml_to_data(xml_file_path, label, node_labels)
